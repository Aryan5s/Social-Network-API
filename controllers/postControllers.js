...
const likePost = async(req , res) => {
    try {
        ...
        return res.status(200).json({msg : `Succesfully Liked the Post with id : ${req.params.id}`})
    } catch (error) {
        console.log(error);
        return res.status(500).json({err : error.message})
    }
...
const unlikePost = async(req , res) => {
    try {
        ...
        return res.status(200).json({msg : `Succesfully unliked the Post with id : ${req.params.id}`})
    } catch (error) {
        console.log(error);
        return res.status(500).json({err : error.message})
    }
